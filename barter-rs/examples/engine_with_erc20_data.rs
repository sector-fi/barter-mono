...
async fn stream_market_event_trades() -> mpsc::UnboundedReceiver<MarketEvent<DataKind>> {
    ...
    let mut erc20_events = get_erc20_events().await.unwrap();
    ...
    tokio::spawn(async move {
        while let Some(event) = erc20_events.recv().await {
            let _ = tx.send(event);
        }
    });
    ...
    while let Some(event) = event_rx.recv().await {
        match event {
            Event::Market(market) => {
                ...
                match market.kind {
                    DataKind::Erc20Transfer(erc20_transfer) => {
                        println!("");
                        println!("got erc20_transfer {erc20_transfer:?}");
                        println!("");
                    }
